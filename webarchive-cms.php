 ...
 *
 * @package    Restorix.CMS
 * @version    Release: 0.1.260115
 * @author     Restorix Team <hello@restorix.io>
 * @telegram   https://t.me/RestorixSupport
 * @messenger  https://m.me/RestorixSupport
 * @copyright  2017-2025 Restorix
 * @license    https://www.gnu.org/licenses/gpl.html GNU GPLv3
 * @link       https://restorix.io/en/restorix-cms
 */
 ...
 */
const ACMS_SAFE_MODE = 1; // Hardened default in this build: blocks custom PHP files; content editing stays fully available.
...
if (php_sapi_name() === 'cli' || $cli = apiRequest()) {
  ...
  switch ($cli['action']) {
    case 'help':
      showWarningJson([
          ...
          'actions' => [
              ...
              'import.warc'                => 'data: (tmp_file | url_file)',
              'meta.get'                   => 'data: name',
    ...
    case 'perform.import.flatfile' :
      ...
      break;
    case 'perform.update' :
      if (updateSystem()) showWarningJson(['action' => $cli['action'], 'status' => 1, 'message' => 'Update performed'], 1);
      else showWarningJson(['action' => $cli['action'], 'status' => 0, 'message' => 'Remote updates are disabled in this build.'], 1);
      break;
    ...
    case 'create.url':
      ...
      break;
    case 'import.warc':
      if (!empty($data['tmp_file']) && file_exists($data['tmp_file'])) {
        $warcFile = $data['tmp_file'];
        $importedWarc = parseWARCFile($warcFile, 'createUrlLocal');
      } elseif (!empty($data['url_file']) && filter_var($data['url_file'], FILTER_VALIDATE_URL)) {
        $warcFile = tempnam(getTempDirectory(), 'archvarix.');
        downloadFile($data['url_file'], $warcFile);
        $importedWarc = parseWARCFile($warcFile, 'createUrlLocal');
        unlink($warcFile);
      } else showWarningJson(['action' => $cli['action'], 'status' => 0, 'message' => 'WARC file is not set or does not exist'], 1);

      if (isset($importedWarc)) showWarningJson(['action' => $cli['action'], 'status' => 1, 'message' => sprintf('%d URLs imported from WARC file.', $importedWarc)], 1);
      break;
    case 'robots.allow' :
    ...
    case 'plugin.uninstall' :
      if (uninstallPlugin($data['name'], !empty($data['forced']))) showWarningJson(['action' => $cli['action'], 'status' => 1, 'message' => "Plugin '{$data['name']}' uninstalled."], 1);
      else showWarningJson(['action' => $cli['action'], 'status' => 0, 'message' => "Could not install plugin '{$data['name']}'."], 1);
      break;
    ...
    case 'change.domain' :
      ...
      break;
    case 'plugin.action':
      $pluginName = basename(str_replace('\\', '/', isset($data['plugin']) ? (string)$data['plugin'] : ''));
      if ($pluginName === '' || $pluginName === '.' || $pluginName === '..') {
        showWarningJson(['action' => $cli['action'], 'status' => 0, 'message' => "Invalid plugin name."], 1);
        break;
      }
      $pluginInterface = $sourcePath . DIRECTORY_SEPARATOR . 'plugins' . DIRECTORY_SEPARATOR . $pluginName . DIRECTORY_SEPARATOR . 'interface.php';
      $pluginData = $data;
...
function analyzeWARCFile($file_path)
{
  $is_gzipped = isGzippedFile($file_path);
  if ($is_gzipped) {
  ...
  if (!$handle) {
    return json_encode(array('error' => 'Could not open file for analysis'));
  }
  ...
  $in_record = false;
  $analyzed_records = 0;
  $max_analysis_records = 1000; // Limit analysis to first 1000 records for speed

  while (true) {
    $line = $is_gzipped ? gzgets($handle) : fgets($handle);

    if ($line === false) {
      break;
    }

    // Detect WARC record start
    if (preg_match('/^WARC\/1\.0/i', $line) || preg_match('/^WARC\/1\.1/i', $line)) {
      if ($in_record && !empty($current_record)) {
        analyzeWARCRecord($current_record, $domains, $record_types, $response_codes,
            $mime_types, $response_records, $earliest_date, $latest_date, $warc_info);
        $analyzed_records++;

        // Limit analysis for performance
        if ($analyzed_records >= $max_analysis_records) {
          break;
        }
      }
      $in_record = true;
  ...
  if ($in_record && !empty($current_record) && $analyzed_records < $max_analysis_records) {
    analyzeWARCRecord($current_record, $domains, $record_types, $response_codes,
        $mime_types, $response_records, $earliest_date, $latest_date, $warc_info);
  }
  ...
  $analysis = array(
      ...
      'records'        => array(
          ...
          'analysis_limited' => $analyzed_records >= $max_analysis_records
      ),
      'warc_info'      => $warc_info,
      'date_range'     => array(),
      'domains'        => $domains,
  ...
  if ($earliest_date && $latest_date) {
    $analysis['date_range'] = array(
        'earliest'           => $earliest_date,
        'latest'             => $latest_date,
        'earliest_formatted' => date('Y-m-d H:i:s', $earliest_date),
        'latest_formatted'   => date('Y-m-d H:i:s', $latest_date)
    );
  }

  // Sort arrays for better presentation
  if (!empty($domains)) {
    arsort($analysis['domains']);
  }
  if (!empty($response_codes)) {
    ksort($analysis['response_codes']);
  }
  if (!empty($mime_types)) {
    arsort($analysis['mime_types']);
  }

  return json_encode($analysis, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
}

function analyzeWARCRecord($record_content, &$domains, &$record_types, &$response_codes,
                           &$mime_types, &$response_records, &$earliest_date, &$latest_date, &$warc_info)
{

  // Extract record type
  if (preg_match('/WARC-Type:\s*([^\s\r\n]+)/i', $record_content, $matches)) {
    $type = strtolower(trim($matches[1]));
    $record_types[$type] = isset($record_types[$type]) ? $record_types[$type] + 1 : 1;

    // Extract WARC info for warcinfo records
    if ($type === 'warcinfo') {
      if (preg_match('/software:\s*([^\r\n]+)/i', $record_content, $sw_matches)) {
        $warc_info['Software'] = trim($sw_matches[1]);
      }
      if (preg_match('/hostname:\s*([^\r\n]+)/i', $record_content, $host_matches)) {
        $warc_info['Created by'] = trim($host_matches[1]);
      }
      if (preg_match('/operator:\s*([^\r\n]+)/i', $record_content, $op_matches)) {
        $warc_info['Operator'] = trim($op_matches[1]);
      }
    }
  }

  // Only analyze response records for detailed stats
  if (!preg_match('/WARC-Type:\s*response/i', $record_content)) {
    return;
  }

  $response_records++;

  // Extract domain from WARC-Target-URI
  if (preg_match('/WARC-Target-URI:\s*([^\s\r\n]+)/i', $record_content, $matches)) {
    $uri = trim($matches[1], '<>');
  ...
  // Extract date
  if (preg_match('/WARC-Date:\s*([^\s\r\n]+)/i', $record_content, $matches)) {
    $date = strtotime($matches[1]);
    if ($date !== false) {
      if ($earliest_date === null || $date < $earliest_date) {
        $earliest_date = $date;
      }
      if ($latest_date === null || $date > $latest_date) {
        $latest_date = $date;
      }
    }
  }

  // Extract HTTP status and content type from HTTP headers
  $parts = preg_split('/\r\n\r\n|\n\n/', $record_content, 3);
  if (count($parts) >= 2) {
    $http_header = $parts[1];

    // HTTP status code
    if (preg_match('/HTTP\/\d\.\d\s+(\d+)/i', $http_header, $matches)) {
      $code = intval($matches[1]);
      $response_codes[$code] = isset($response_codes[$code]) ? $response_codes[$code] + 1 : 1;
    }

    // MIME type
    if (preg_match('/Content-Type:\s*([^;\r\n]+)/i', $http_header, $matches)) {
      $mime = trim($matches[1]);
      $mime_types[$mime] = isset($mime_types[$mime]) ? $mime_types[$mime] + 1 : 1;
    }
  }
}
...
function getLoginAttemptsFile()
{
  global $sourcePath;
  $dir = (is_string($sourcePath) && strlen($sourcePath) && is_dir($sourcePath)) ? $sourcePath : __DIR__;
  return $dir . DIRECTORY_SEPARATOR . '.acms.login.attempts.json';
}
...
{
  @file_put_contents(getLoginAttemptsFile(), json_encode($attempts), LOCK_EX);
}
...
function isLoginRateLimited($ip)
{
  ...
  $key = sha1($ip);
  if (empty($attempts[$key]) || !is_array($attempts[$key])) return false;
  $now = time();
  $recent = array_filter($attempts[$key], function ($time) use ($now) {
    return is_numeric($time) && $time > $now - 600;
  });
...
function recordFailedLogin($ip)
{
  $attempts = getLoginAttempts();
  $now = time();
  foreach ($attempts as $key => $times) {
    if (!is_array($times)) {
      unset($attempts[$key]);
      continue;
    }
    $times = array_values(array_filter($times, function ($time) use ($now) {
      return is_numeric($time) && $time > $now - 600;
    }));
    if (empty($times)) unset($attempts[$key]);
    else $attempts[$key] = $times;
  }
  $key = sha1($ip);
  if (empty($attempts[$key])) $attempts[$key] = [];
  $attempts[$key][] = $now;
...
function checkAccess()
{
  ...
  if (isset($_POST['password']) && strlen($_POST['password'])) {
    $loginIp = !empty($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : 'cli';
    if (isLoginRateLimited($loginIp)) {
      error_log("Restorix CMS login rate-limited; TIME: " . date('c') . "; IP: " . $loginIp . PHP_EOL);
      return false;
    }
    if (strlen($ACMS['ACMS_SAFE_PASSWORD']) && password_verify($_POST['password'], $ACMS['ACMS_SAFE_PASSWORD'])) {
      regenerateSessionId();
      clearFailedLogins($loginIp);
      $_SESSION['archivarix.logged'] = password_hash($ACMS['ACMS_SAFE_PASSWORD'], PASSWORD_DEFAULT);
      $_SESSION['archivarix.safe_mode'] = 1;
      return true;
    }
    if (strlen(ACMS_SAFE_PASSWORD) && hash_equals(ACMS_SAFE_PASSWORD, $_POST['password'])) {
      regenerateSessionId();
      clearFailedLogins($loginIp);
    ...
    if (strlen(ACMS_PASSWORD) && hash_equals(ACMS_PASSWORD, $_POST['password'])) {
      regenerateSessionId();
      clearFailedLogins($loginIp);
    ...
    error_log("Restorix CMS login failed; TIME: " . date('c') . "; IP: " . $_SERVER['REMOTE_ADDR'] . PHP_EOL);
    return false;
  }
...
function isGzippedFile($file_path)
{
  $handle = fopen($file_path, 'rb');
  if (!$handle) {
    return false;
  ...
  $magic_bytes = fread($handle, 3);
  fclose($handle);

  return (strlen($magic_bytes) >= 2 &&
      ord($magic_bytes[0]) === 0x1f &&
      ord($magic_bytes[1]) === 0x8b);
}
...
{
  // Remote plugin installs are disabled in this build: plugins were
  // downloaded over the network without any signature verification
  // (supply-chain risk). Never write files fetched from the network here.
  addWarning(L('Remote updates are disabled in this build.'), 4, L('Plugins'));
  return false;
}
...
function cliImportLoaderJson($data)
{
  ...
  fileWrite($loaderFilename, jsonify($LOADER));
  return true;
}
...
function convertUTF8($taskOffset = 0)
{
  ...
  $stats = array_merge(['pages' => 0, 'processed' => 0, 'total' => 0, 'time' => 0], unserialize($taskStats, ['allowed_classes' => false]));
...
function copyUrl($metaDataNew)
{
  ...
  $destination = $sourcePath . DIRECTORY_SEPARATOR . $metaDataNew['folder'] . DIRECTORY_SEPARATOR . $metaDataNew['filename'];
  rename($metaDataNew['tmp_file_path'], $destination);
  // set 0644 for destination
...
function createCustomRule($input)
{
  ...
  fileWrite($loaderFile, jsonify($LOADER));
}
...
<p>The requested URL was not found on this server.</p>
  ...
  while ($hostname = $stmt->fetchColumn()) {
    ...
    $metaData = getMetaData($rowid);
    $file = $sourcePath . DIRECTORY_SEPARATOR . $metaData['folder'] . DIRECTORY_SEPARATOR . $metaData['filename'];
    fileWrite($file, $page404);
...
function createTemplatePage($input)
{
  ...
  if ($rowid) {
    $url = getMetaData($rowid);
    $file = $sourcePath . DIRECTORY_SEPARATOR . $url['folder'] . DIRECTORY_SEPARATOR . $url['filename'];
    // [TODO] convertEncoding() back
    fileWrite($file, $template['content']);
...
function createUrl($input)
{
  ...
  if (isset($input['request_uri'])) $input['path'] = $input['request_uri'];
  if (isset($input['mimetype'])) $input['mime'] = $input['mimetype'];
  ...
  if ($createID) {
    ...
    if (!empty($_FILES['create_file']['tmp_name'])) {
      ...
      fileWrite($file, base64_decode($input['content']));
    } else {
      touch($file);
    }
...
{
  if (empty($agent)) $agent = "Restorix-CMS/" . ACMS_VERSION . " (+https://restorix.io/en/restorix-cms)";
  // Follow redirects manually (max 5) and validate every hop with isSafeRemoteUrl()
  // so a 301/302 cannot bounce the request to a private/internal address (SSRF).
  $maxRedirects = 5;
  for ($hop = 0; ; $hop++) {
    ...
    if (!in_array($status, [301, 302, 303, 307, 308], true)) return $body;
    if (empty($location) || $hop >= $maxRedirects) return false;
...
function dataLogo()
{
  return "data:image/svg+xml;base64,";
}
...
function detectPagesDepth()
{
  ...
  $stats = array_merge([
      ...
      'total'             => 0,
      'time'              => 0,
  ], unserialize($taskStats, ['allowed_classes' => false]));
...
function doSearchReplaceCode($params, $taskOffset = 0)
{
  ...
  $stats = array_merge(['pages' => 0, 'result' => ['total_urls' => 0, 'total_matches' => 0, 'limit_reached' => 0], 'total_matches' => 0, 'processed' => 0, 'replaced' => 0, 'removed' => 0, 'total' => 0, 'time' => 0], unserialize($taskStats, ['allowed_classes' => false]));
...
function downloadFile($url, $dest, $taskOffset = 0, $userAgent = null, $headers = [])
{
  global $taskIncomplete;
  global $taskIncompleteOffset;
  global $taskStats;
  ...
  $stats = array_merge(['size' => 0, 'pages' => 0, 'processed' => 0, 'total' => 0, 'time' => 0], unserialize($taskStats, ['allowed_classes' => false]));

  $fileHandle = is_resource($dest) ? $dest : ($taskOffset ? fopen($dest, 'a+') : fopen($dest, 'w+'));
  $ch = null;
  $return = false;
  // Follow redirects manually (max 5) and validate every hop with isSafeRemoteUrl()
  // so a 301/302 cannot bounce the download to a private/internal address (SSRF).
  $maxRedirects = 5;
  for ($hop = 0; ; $hop++) {
    if (!isSafeRemoteUrl($url)) {
      addWarning(sprintf(L("Could not download %s. The URL is not allowed."), basename((string) parse_url($url, PHP_URL_PATH))), 4, L('An import tool'));
      if (!is_resource($dest) && is_resource($fileHandle)) fclose($fileHandle);
      return false;
    }
    $location = null;
    $options = [
        ...
        CURLOPT_USERAGENT      => $userAgent ?: "Restorix-CMS/" . ACMS_VERSION . " (+https://restorix.io/en/restorix-cms)",
        CURLOPT_SSL_VERIFYHOST => 2,
        CURLOPT_SSL_VERIFYPEER => true,
        ...
        CURLOPT_WRITEFUNCTION  => function ($ch, $data) use ($fileHandle) {
          // Discard bodies of redirect responses so they never reach the destination file.
          $status = (int) curl_getinfo($ch, CURLINFO_RESPONSE_CODE);
          if (in_array($status, [301, 302, 303, 307, 308], true)) return strlen($data);
    ...
    $return = curl_exec($ch);
    $status = (int) curl_getinfo($ch, CURLINFO_RESPONSE_CODE);
    if ($return !== false && in_array($status, [301, 302, 303, 307, 308], true)) {
      curl_close($ch);
      $ch = null;
      $resolvedUrl = empty($location) ? false : resolveRedirectUrl($location, $url);
      if ($resolvedUrl === false || $hop >= $maxRedirects) {
        addWarning(sprintf(L("Could not download %s. Too many redirects or an invalid redirect URL."), basename((string) parse_url($url, PHP_URL_PATH))), 4, L('An import tool'));
        if (!is_resource($dest) && is_resource($fileHandle)) fclose($fileHandle);
        return false;
      }
      $url = $resolvedUrl;
...
function downloadFromSerial($uuid, $taskOffset = 0)
{
  ...
  $filename = "{$uuid}.zip";
  downloadFile(
      'https://download.restorix.io/restores/' . $uuid[0] . '/' . $uuid[1] . '/' . $uuid . '.zip' . '?uid=' . sha1(__DIR__),
...
{
  // FILTER_FLAG_NO_PRIV_RANGE rejects 10/8, 172.16/12, 192.168/16 and IPv6 fc00::/7.
  // FILTER_FLAG_NO_RES_RANGE rejects 0/8, 127/8, 169.254/16, 224/4, 240/4 and IPv6 ::1, ::/128, ::ffff:0:0/96, fe80::/10.
  if (filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_NO_PRIV_RANGE | FILTER_FLAG_NO_RES_RANGE)) {
    // IPv4-compatible IPv6 addresses (::/96) can smuggle a private IPv4 past the flags above
    if (filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6)) {
      $packed = @inet_pton($ip);
      if ($packed !== false && substr($packed, 0, 12) === str_repeat("\0", 12)) {
        $ipv4 = @inet_ntop(substr($packed, 12));
        if ($ipv4 !== false) {
          return !filter_var($ipv4, FILTER_VALIDATE_IP, FILTER_FLAG_NO_PRIV_RANGE | FILTER_FLAG_NO_RES_RANGE);
        }
      }
    }
    return false;
...
function isSafeRemoteUrl($url)
{
  ...
  if (filter_var($host, FILTER_VALIDATE_IP)) return !isPrivateIpAddress($host);
  $addresses = @gethostbynamel($host);
  if (empty($addresses)) return false;
  foreach ($addresses as $ip) {
    if (isPrivateIpAddress($ip)) return false;
  }
  return true;
}
...
{
  // Resolve a Location header against the request URL (RFC 3986 section 5.2,
  // simplified). Dot segments are left as-is: curl normalizes them when sending
  // and isSafeRemoteUrl() re-validates scheme and host of the result.
  $location = trim($location);
  if (!strlen($location)) return false;
  if (preg_match('~^https?://~i', $location)) return $location;
  $baseParts = parse_url($base);
  if (empty($baseParts['scheme']) || empty($baseParts['host'])) return false;
  if (substr($location, 0, 2) === '//') return strtolower($baseParts['scheme']) . ':' . $location;
  $prefix = strtolower($baseParts['scheme']) . '://';
  if (isset($baseParts['user'])) {
    $prefix .= $baseParts['user'];
    if (isset($baseParts['pass'])) $prefix .= ':' . $baseParts['pass'];
    $prefix .= '@';
  }
  $prefix .= $baseParts['host'];
  if (isset($baseParts['port'])) $prefix .= ':' . $baseParts['port'];
  if ($location[0] === '/') return $prefix . $location;
  if ($location[0] === '?') return $prefix . (isset($baseParts['path']) ? $baseParts['path'] : '/') . $location;
  if ($location[0] === '#') return preg_replace('~#.*$~', '', $base) . $location;
  $basePath = isset($baseParts['path']) ? $baseParts['path'] : '/';
  $slashPos = strrpos($basePath, '/');
  $baseDir = ($slashPos !== false) ? substr($basePath, 0, $slashPos + 1) : '/';
  return $prefix . $baseDir . $location;
}
  ...
  for ($i = 0; $i < $zip->numFiles; $i++) {
    ...
    if (preg_match('~^[.]content[.][0-9a-z]+$~i', basename($zip->statIndex($i)['name'])) && $zip->statIndex($i)['size'] == 0) {
      ...
      if (isset($import['info']['settings']['uuidg'])) {
        $import['screenshot'] = 'https://download.restorix.io/screenshots/' . $import['info']['settings']['uuidg'][0] . '/' . $import['info']['settings']['uuidg'][1] . '/' . $import['info']['settings']['uuidg'] . '_THUMB.jpg';
        $import['url'] = 'https://restorix.io/' . getLang() . '/status/' . $import['info']['settings']['uuidg'] . '/';
      } else {
        $import['url'] = 'https://restorix.io/' . getLang() . '/status/' . $import['info']['settings']['uuid'] . '/';
      }
...
function getLoaderInfo()
{
  ...
  $filenames = ['index.php', 'webarchive.php'];
...
{
  global $ACMS;
  if (empty($input['external_url']) || !isSafeRemoteUrl($input['external_url'])) return;
  //addWarning( $input );
  $tmp_download = tempnam(getTempDirectory(), 'archivarix.');
  $acmsTimeout = $ACMS['ACMS_TIMEOUT'];
  $ACMS['ACMS_TIMEOUT'] = 0;
  // get hostname of $input['external_url']
  $hostname = parse_url($input['external_url'], PHP_URL_HOST);
  $response = downloadFile($input['external_url'], $tmp_download, 0, $input['user_agent'], ["Referer: https://{$hostname}/", "Origin: https://{$hostname}"]);
...
function importFunMD($params, $taskOffset = 0)
{
...
function importPerform($importFileName, $importSettings, $taskOffset = 0)
{
  ...
  if (zipArchiveHasUnsafePaths($zip)) {
    $zip->close();
    unlink($import['tmp_database']);
    return;
  }
  ...
  while ($url = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    fileWrite($url['tmp_file_path'], $zip->getFromName($url['zip_file_path']));
  ...
  if (!empty($importSettings['acms_settings']) && !inSafeMode()) fileWrite($sourcePath . DIRECTORY_SEPARATOR . '.acms.settings.json', $zip->getFromName($import['zip_source_path'] . '.acms.settings.json'));
  if (!empty($importSettings['loader_settings']) && !inSafeMode()) fileWrite($sourcePath . DIRECTORY_SEPARATOR . '.loader.settings.json', $zip->getFromName($import['zip_source_path'] . '.loader.settings.json'));
  ...
  if (!empty($importSettings['templates']) && !empty($import['templates'])) {
    ...
    while ($template = $stmt->fetch(PDO::FETCH_ASSOC)) {
      if (array_key_exists($template['name'], $import['templates'])) {
        ...
        fileWrite($templatesPath . DIRECTORY_SEPARATOR . $template['name'] . ".html", $zip->getFromName($import['templates'][$templateName]['path']));
      }
    }
  }
...
function loadLocalization($languageCode)
{
  $localization = [
      'ru' => [
          ...
          '0244e20fcb5b41b39cd741cf9ccd1d522549e28b' => 'исходный адрес из Restorix',
          ...
          '08ac72820a2d55f9e50ecbe836ae9fe7258e3464' => 'Внимание! По соображениям безопасности мы рекомендуем переименовать дефолтное имя файла "webarchive-cms.php".',
          ...
          '16384ffc96a04ff099a875878c8c470acd0e639d' => 'Импортировать настройки Restorix Лоадера',
          ...
          '3e682693d76c77ad239049b5b2f7eff67983c3ce' => 'Имя файла скрипта Restorix CMS было переименовано в новое значение.',
          ...
          '4a0a1b4d693e68eb4f31753364b892ff2e1588ea' => 'Restorix CMS использует библиотеку %s для регулярных выражений',
          ...
          '5cc5a245f0b1afe6cae73bd9bdebd37e9c28475b' => 'Restorix Лоадер',
          ...
          '6df5f48b66333bf17e195f87bb9f6b3578b06c75' => 'Импорт/переустановка веб-сайтов, созданных Restorix.',
          ...
          '76d2facfd0cf8e178a0831cc5a2112ff0e2f4ba9' => 'Импортируйте сайты, созданные Restorix.',
          ...
          '8f5502d0b3c1d0293cf2fdd8a9d686a5c3956d32' => 'Внимание! Если вы не выберите флажок, то файл index.php от %s будет перезаписан Лоадером Restorix.',
          ...
          '95927924973cc508fe9322672c45e9f0aed2749c' => 'Это действие установит Restorix Лоадер и заменит содержимое .htaccess.',
          ...
          'b762993dcde7f686d82bb86cbcfc061cedbc302f' => 'Включая настройки Restorix CMS',
          ...
          'bd9f6b614417eefea1df17af14201bd62928ea1b' => 'Проверит и обновит Restorix CMS и Restorix Лоадер на последние версии.',
          ...
          'd793b42c593a94dc19ca8aac2fc9a07d8fbf74d9' => 'Импортировать настройки Restorix CMS',
...
function parseWARCFile($file_path, $call = 'createUrl')
{
  $is_gzipped = isGzippedFile($file_path);
  if ($is_gzipped) {
    // Use gzopen for gzipped files
    $handle = gzopen($file_path, 'rb');
    if (!$handle) {
      echo "Error: Could not open gzipped file: " . $file_path . "\n";
      exit(1);
    }
  } else {
    ...
    $handle = fopen($file_path, 'rb');
    if (!$handle) {
      echo "Error: Could not open file: " . $file_path . "\n";
      exit(1);
    }
  }
  ...
  while (true) {
    ...
    if (preg_match('/^WARC\/1\.0/i', $line) || preg_match('/^WARC\/1\.1/i', $line)) {
      if ($in_record && !empty($current_record)) {
        ...
        processWARCRecord($current_record, $call);
        $record_count++;
    ...
    if (strlen($buffer) > 10 * 1024 * 1024) { // 10MB
      $buffer = substr($buffer, -5 * 1024 * 1024); // Keep last 5MB
    }
  }
...
function processWARCRecord($record_content, $call)
{
  ...
  if (preg_match('/WARC-Target-URI:\s*([^\s\r\n]+)/i', $record_content, $matches)) {
    ...
    // Remove angle brackets if present (common in WARC files)
    $uri = trim($uri, '<>');

    $parsed_url = parse_url($uri);

    if (isset($parsed_url['host'])) {
      $data['hostname'] = $parsed_url['host'];
    }

    $data['request_uri'] = isset($parsed_url['path']) ? $parsed_url['path'] : '/';
    if (isset($parsed_url['query'])) {
      $data['request_uri'] .= '?' . $parsed_url['query'];
    }
  }
  ...
  // Split record into headers and content
  $parts = preg_split('/\r\n\r\n|\n\n/', $record_content, 3);
  ...
  if (preg_match('/Content-Type:\s*([^;\r\n]+)(?:;\s*charset=([^;\r\n]+))?/i', $http_header, $matches)) {
    $data['mimetype'] = trim($matches[1]);
    if (isset($matches[2])) {
      $data['charset'] = trim($matches[2]);
    }
  }

  // Extract Location for redirects
  if (($data['code'] == 301 || $data['code'] == 302 || $data['code'] == 307 || $data['code'] == 308)
      && preg_match('/Location:\s*([^\r\n]+)/i', $http_header, $matches)) {
    $redirect_url = trim($matches[1]);

    // Check if redirect URL is for the same domain
    $redirect_parsed = parse_url($redirect_url);

    if (isset($redirect_parsed['host']) && $redirect_parsed['host'] === $data['hostname']) {
      // Same domain - store only the path and query
      $data['redirect'] = isset($redirect_parsed['path']) ? $redirect_parsed['path'] : '/';
      if (isset($redirect_parsed['query'])) {
        $data['redirect'] .= '?' . $redirect_parsed['query'];
      }
      if (isset($redirect_parsed['fragment'])) {
        $data['redirect'] .= '#' . $redirect_parsed['fragment'];
      }
    } else {
      // Different domain or relative URL - store as is
  ...
  // Call the user function with the extracted data
  if (function_exists($call)) {
    $result = call_user_func($call, $data);
  }
  //print_r($data);
}
...
{
  // Remote loader installs are disabled in this build: the loader was
  // downloaded over the network without any signature verification
  // (supply-chain risk). Never write files fetched from the network here.
  addWarning(L('Remote updates are disabled in this build.'), 4, L('Restorix Loader'));
  return false;
}
...
function removeBrokenImages($taskOffset = 0)
{
  ...
  $stats = array_merge(['images' => 0, 'pages' => 0, 'processed' => 0, 'total' => 0, 'time' => 0], unserialize($taskStats, ['allowed_classes' => false]));
  ...
  while ($url = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    if ($updatedImages) {
      ...
      fileWrite($file, convertEncoding(convertHtmlEncoding(html_entity_decode($dom->saveHTML()), $url['charset'], 'utf-8'), $url['charset'], 'utf-8'));
      updateFilesize($url['rowid'], filesize($file));
...
function removeBrokenLinks($taskOffset = 0)
{
  ...
  $stats = array_merge(['links' => 0, 'pages' => 0, 'processed' => 0, 'total' => 0, 'time' => 0], unserialize($taskStats, ['allowed_classes' => false]));
...
function removeExternalLinks($white_list, $taskOffset = 0)
{
  global $taskIncomplete;
  global $taskIncompleteOffset;
  global $taskStats;
  ...
  $stats = array_merge(['links' => 0, 'pages' => 0, 'processed' => 0, 'total' => 0, 'time' => 0], unserialize($taskStats, ['allowed_classes' => false]));
  // domains white list
  $white_list = array_filter(array_map('trim', preg_split('~[,\s]+~', $white_list)));
  foreach ($white_list as $key => $url) {
    if (parse_url($url, PHP_URL_HOST)) $white_list[$key] = strtolower(convertIdnToAscii(parse_url($url, PHP_URL_HOST)));
  }
  ...
  while ($url = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    for ($n = $linkTags->length - 1; $n >= 0; --$n) {
      ...
      $hrefHostname = strtolower(convertIdnToAscii(parse_url($hrefAbsolute, PHP_URL_HOST)));
      if (preg_match('~^([-a-z0-9.]+\.)?' . preg_quote($uuidSettings['domain'], '~') . '$~i', $hrefHostname)) continue;
      if (in_array($hrefHostname, $white_list)) continue;
      $updatedLinks++;
      while ($linkTags->item($n)->hasChildNodes()) {
        $linkTagChild = $linkTags->item($n)->removeChild($linkTags->item($n)->firstChild);
        $linkTags->item($n)->parentNode->insertBefore($linkTagChild, $linkTags->item($n));
...
function removeVersionControl($taskOffset = 0)
{
  ...
  $stats = array_merge(['urls' => 0, 'pages' => 0, 'replaces' => 0, 'processed' => 0, 'total' => 0, 'time' => 0], unserialize($taskStats, ['allowed_classes' => false]));
...
function saveTemplateFile($name)
{
  ...
  $file = $templatesPath . DIRECTORY_SEPARATOR . $metaData['name'] . '.html';
  fileWrite($file, $content);
  responseAjax();
}
...
function setAcmsSettings($settings, $filename = null)
{
  ...
  fileWrite($filename, jsonify($ACMS));
}
...
function setLanguage($lang)
{
  ...
  if (in_array($lang, ['en', 'ru'])) {
    ...
    $externalLocalization = json_decode(curlContent("https://restorix.io/download/cms/locales/archivarix.cms.{$lang}.json" . '?uid=' . sha1(__DIR__)), true);
...
function setLoaderSettings($settings, $filename = null)
{
  ...
  $LOADER = array_merge($LOADER, $settings);
  fileWrite($filename, jsonify($LOADER));
}
...
function uninstallPlugin($name)
{
  global $sourcePath;
  if (!preg_match('~^[-\w]+$~i', $name)) return false;
  if ($installedPlugin = isPluginInstalled($name)) {
...
function zipArchiveHasUnsafePaths($zip)
{
  for ($i = 0; $i < $zip->numFiles; $i++) {
...
function updateHtaccessFile($content)
{
  ...
  fileWrite($htaccessPath, $content);
}
...
{
  // Remote self-updates are disabled in this build: the updater downloaded
  // code over the network and overwrote the running CMS file without any
  // signature verification (supply-chain risk). Never write files fetched
  // from the network here.
  addWarning(L('Remote updates are disabled in this build.'), 4, L('System update'));
  return false;
}
...
$urlOffsets = !empty($_POST['urlOffsets']) ? unserialize($_POST['urlOffsets'], ['allowed_classes' => false]) : [];
$urlsTotal = [];
...
if (isset($_POST['action']) &&
  ...
  if (!inSafeMode()) {
    switch ($_POST['action']) {
      ...
      case 'update.urls.meta' :
        ...
        break;
      case 'import.md.zip' :
        importFunMD($_POST['params'], $taskIncompleteOffset);
        addWarning(L('Done.'), 1);
        $section = 'tools';
        break;
      case 'edit.htaccess' :
  ...
  switch ($_POST['action']) {
    ...
    case 'external.links.update' :
      ...
      break;
    case 'external.links.remove' :
      $removedLinks = removeExternalLinks($_POST['white_list'], $taskIncompleteOffset);
    ...
    case 'scan.external.resources':
      ...
      break;
    case 'plugin.action' :
      $pluginName = basename(str_replace('\\', '/', isset($_POST['plugin']) ? (string)$_POST['plugin'] : ''));
      if ($pluginName === '' || $pluginName === '.' || $pluginName === '..') {
        addWarning(L('Invalid plugin name.'), 4, L('Plugins'));
        $section = 'settings';
        $subSection = 'plugins';
        break;
      }
...
?>
  ...
  <head>
    ...
    <link rel="icon"
          href="data:image/svg+xml;base64,">
    <style>
        body {
    ...
    <div class="container">
      ...
      <div class="card mb-3 border-0">
        ...
        <div class="card-body border border-top-0">
          <div class="">
            <?php if (basename(__FILE__) == 'webarchive-cms.php') { ?>
              <div class="alert alert-warning d-md-flex align-items-center">
                <div class="mr-auto pr-md-2">
                  <?=L('Attention! For security reasons, we recommend to rename the default name of the "webarchive-cms.php" file.')?> <?=L('You can do this at any time manually on the server, or by pressing a button that will rename the file to a random name.')?> <?=L('Do not forget to bookmark the new address after renaming!')?>
                </div>
            ...
            <?php } else { ?>
              <div class="alert alert-success">
                &#x1f44d <?=L('The filename of Restorix CMS script has been renamed to a non-default value.')?> <?=L('Do not forget to bookmark the address of this page!')?>
              </div>
            <?php } ?>
                    ...
                    <form action="" method="post" id="form_import_<?=$import['id']?>"
                        ...
                        <div class="alert alert-warning">
                          <div class="custom-control custom-checkbox mb-0">
                            ...
                            <small class="form-text"><i
                                  class="fas fa-exclamation-triangle fa-fw"></i> <?=sprintf(L('Attention! If you do not select the checkbox, then the index.php file of %s will be overwritten with Restorix Loader.'), $integrationPrerequisite)?>
                            </small>
                          </div>
                        </div>
                      <?php } ?>
                      ...
                      <?php if ($import['acms_settings']) { ?>
                        <div class="custom-control custom-checkbox mb-0">
                          ...
                          <label class="custom-control-label"
                                 for="check_acms_settings_<?=$import['id']?>"><?=L('Import Restorix CMS settings')?></label>
                        </div>
                      <?php } ?>
                      ...
                      <?php if ($import['loader_settings']) { ?>
                        <div class="custom-control custom-checkbox mb-0">
                          ...
                          <label class="custom-control-label"
                                 for="check_loader_settings_<?=$import['id']?>"><?=L('Import Restorix Loader settings')?></label>
                        </div>
                      <?php } ?>
  ...
  if ($section == 'install' && $taskIncomplete) { ?>
    <div class="pt-3 container">
      ...
      <?php
      $stats = unserialize($taskStats, ['allowed_classes' => false]);
                  ...
                  <ul class="nav nav-tabs card-header-tabs" role="tablist"
                    ...
                    <li class="nav-item">
                      <a class="nav-link <?=($subSection == 'acms') ? 'active' : ''?>"
                         ...
                         aria-selected="<?=($subSection == 'acms') ? 'true' : 'false'?>">Restorix
                        CMS</a>
                    </li>
                    <li class="nav-item">
                      <a class="nav-link <?=($subSection == 'loader') ? 'active' : ''?>"
                         ...
                         aria-selected="<?=($subSection == 'loader') ? 'true' : 'false'?>"><?=L('Restorix Loader')?></a>
                    </li>
                    <li class="nav-item">
            ...
            if (inExpertMode() && $section == 'import.urls') {
              ...
              <form action="" method="post" class="import-external-urls-wrapper">
                ...
                <div class="input-group mb-3">
                  ...
                  <div class="input-group-append">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                      ...
                      <a class="dropdown-item put-custom-rule cursor-link"
                         data-useragent="Restorix-CMS/<?=ACMS_VERSION?> (+https://restorix.io/en/restorix-cms)">Restorix
                        CMS</a>
                    </div>
                  </div>
                </div>
                <div class="custom-control custom-checkbox">
                  ...
                  <label class="custom-control-label"
                         for="external_urls_web_arvchive"><?=L('Grab from Restorix')?></label>
                </div>
                <div class="custom-control custom-checkbox">
                ...
                <div class="col mb-4">
                  <div class="card h-100 shadow">
                    ...
                    <div class="card-body small">
                      <?=L('Import/reinstall websites created by Restorix.')?>
                          ...
                          <div class="row">
                            ...
                            <div class="col-12 text-center align-self-center">
                              ...
                              <div class="form-group text-left">
                                <label><?=L('Domains white-list')?></label>
                                <textarea class="form-control form-control-sm w-100"
                                          name="white_list" value=""
                                          placeholder="<?=L('Domains white-list')?>"
                                          autocorrect="off" autocapitalize="none" form="form_remove_external_links"></textarea>
                              </div>
                    ...
                    <div class="col-12 col-md-3">
                      <?php if (true && inExpertMode()) { ?>
                        <form action="" method="post" class="needs-validation" novalidate>
                          ...
                          <button class="btn btn-block btn-primary mb-3" type="submit" name="action" value="import.flatfile.zip"><i class="fas fa-file-import fa-fw"></i> <?=L('Import')?></button>
                          <button class="btn btn-block btn-primary mb-3" type="submit" name="action" value="import.md.zip"><i class="fas fa-file-import fa-fw"></i> <?=L('Import MD')?></button>
                        </form>
                      <?php } ?>
                      <form action="" method="post">
                        <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                  ...
                  <div class="collapse mt-2 bg-light" id="exportSettings">
                    ...
                    <div class="custom-control custom-checkbox">
                      ...
                      <input id="export_settings_cms"
                             ...
                             name="settings[acms_settings]" value="1">
                      <label class="custom-control-label"
                             for="export_settings_cms"><?=L('Include Restorix CMS settings')?></label>
                    </div>
                    <div class="custom-control custom-checkbox">
                      <input type="hidden" name="settings[loader_settings]" value="0">
                      <input id="export_settings_loader"
                             ...
                             name="settings[loader_settings]" value="1">
                      <label class="custom-control-label"
                             for="export_settings_loader"><?=L('Include Restorix Loader settings')?></label>
                    </div>
                    <div class="custom-control custom-checkbox">
                ...
                <div class="text-center mb-3">
                  <form action="" method="post" id="form_confirm_remove" class="form-wait">
                    ...
                    <input type="hidden" name="regex"
                           value="<?=htmlspecialchars((string)$_POST['regex'], ENT_QUOTES, 'UTF-8')?>"/>
                    <input type="hidden" name="xsrf" value="<?=getXSRF()?>">
                    <input type="hidden" name="action"
                           value="<?=htmlspecialchars((string)$_POST['action'], ENT_QUOTES, 'UTF-8')?>"/>
                    <input type="hidden" name="filterValue" class="filterValue"
                           value="<?=htmlspecialchars($filterValue)?>"/>
                    <input type="hidden" name="urlOffsets" class="urlOffsets"
                           value="<?=htmlspecialchars(serialize($urlOffsets))?>"/>
                    <input type="hidden" name="type"
                           value="<?=htmlspecialchars((string)$_POST['type'], ENT_QUOTES, 'UTF-8')?>"/>
                    <input type="hidden" name="text_files_search"
                           value="<?=htmlspecialchars((string)$_POST['text_files_search'], ENT_QUOTES, 'UTF-8')?>"/>
                    ...
                    <?php if (!empty($_POST['replace_url'])) { ?>
                      <input type="hidden" name="replace_url"
                             value="<?=htmlspecialchars((string)$_POST['replace_url'], ENT_QUOTES, 'UTF-8')?>"/>
                    <?php } ?>
                ...
                <div class="text-center mb-3">
                  <form action="" method="post" id="form_confirm_replace" class="form-wait">
                    ...
                    <input type="hidden" name="reserved_vars"
                           value="<?=!empty($_POST['reserved_vars']) ? htmlspecialchars((string)$_POST['reserved_vars'], ENT_QUOTES, 'UTF-8') : 0?>"/>
            ...
            <?php if ($section == 'main' && $accessAllowed) {
                      ...
                      <table class="table table-sm table-hover mb-3">
                        ...
                        <tr>
                          ...
                          <td class="text-monospace">
                            <a href="https://restorix.io/<?=getLang()?>/status/<?=($uuidSettings['uuidg'] ? $uuidSettings['uuidg'] : $uuidSettings['uuid'])?>/"
              ...
              <div class="card border-0 shadow mb-3">
                ...
                <div class="card-body small">
                  <div class="row">
                    <div class="col-12 col-md-6">
                      <?=L('This tool checks and updates Restorix CMS, Restorix Loader to the latest version.')?> <?=L('If you have manually edited the source code, all changes will be lost.')?> <?=L('CMS and Loader settings that were set using the Settings menu will not be affected.')?></p>
                      <table class="table table-hover table-sm table-responsive mb-0">
                        ...
                        <tr>
                          <td class="text-nowrap">Restorix CMS</td>
                        ...
                        <tr>
                          <td class="text-nowrap"><?=L('Restorix Loader')?><?=$loaderInfo['integration'] ? ' + <i class="fab fa-' . $loaderInfo['integration'] . ' fa-fw"></i>' : ''?></td>
                          ...
                          <div class="col">
                            <form action="" method="post" id="form_install_loader"
                              ...
                              <button type="button"
                                      ...
                                      data-message="<?=L('This action will install Restorix Loader and replace .htaccess content.')?>" <?=!empty($missingExtensions) ? ' disabled' : ''?>><?=L('Install missing Loader')?></button>
                            </form>
                          </div>
                        <?php } ?>
                        <div class="col">
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              <div class="alert alert-primary px-3 py-2 d-flex align-items-center">
                <?=sprintf(L('Restorix CMS uses %s library for Regex'), version_compare(PHP_VERSION, '7.3', '>=') ? 'PCRE2' : 'PCRE')?>
            ...
            <div class="modal-body">
              ...
              <div class="row">
                <div class="col-12 mb-3">
                  ...
                  <br>
                  <kbd>%%url_original%</kbd> <?=L('the source url from the Restorix')?><br>
  ...
  <script language="JavaScript">
      ...
      <?php if($taskIncomplete) { ?>
      $('#pleaseWaitDialog .message').html('<?=htmlspecialchars(sprintf(L('Processed: %s'), number_format(unserialize($taskStats, ['allowed_classes' => false])['pages']), 2), ENT_QUOTES)?>');
      ...
      $('#checkUpdate').on('click touch', function () {
          $.getJSON("https://restorix.io/<?=getLang()?>/cms/?ver=<?=ACMS_VERSION?><?=(!empty($loaderInfo['version']) ? '&loader=' . $loaderInfo['version'] : '')?>&uid=<?=sha1(__DIR__)?>", function (json) {
...
L( 'Read instructions' );
L( 'Import websites created by Restorix.' );
